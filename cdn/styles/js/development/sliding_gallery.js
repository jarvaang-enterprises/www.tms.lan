var preloadSlides = 10;                // Number of slides to preload before showing gallery
var loadingMessageDelay = 1500;       // How long to wait before showing loading message (in ms)
...
var slideHeights = new Array();       // Holds the heights (in pixels)
...
function init() {
  ...
  buttonHeight = $('#leftBtn').css('height');
  $('#leftBtn').css('height',0);
  $('#rightBtn').css('height',0);
  ...
  $('#caption').css({
    top: $('#gallery').height() - slides[currentSlide].height() + 'px',
  });
...
function handleSlideLoad() {
  ...
  if ( loading ) {
    ...
    if ( preloaded == preloadSlides || preloaded == totalSlides ) {
      ...
      $('#leftBtn').css('height',buttonHeight);
      $('#rightBtn').css('height',buttonHeight);
      $('#rightBtn').show();
      addSlideHover();
      loading = false;
      $('#loading').css({
...
function centreCurrentSlide() {
  ...
  $('#gallery').css( 'left', offset - 100 + 'px');
}
